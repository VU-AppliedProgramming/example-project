...
document.addEventListener('DOMContentLoaded', () => {
  /* ─────────────────────────────── CONSTANTS ───────────────────────────── */
  const DEFAULT_IMAGE     = '/static/images/temp.png';
  ...
  function displaySearchResults(results) {
    ...
    results.forEach(r => {
      ...
      const imgUrl = r.image || DEFAULT_IMAGE;
      card.innerHTML = `
      ...
      imgEl.addEventListener('click', () => fetchRecipeDetails(r.id));
  ...
  function displayRandomRecipe(recipe) {
    const img = recipe.image || DEFAULT_IMAGE;
    const stats = `
  ...
  function fetchRecipeDetails(id) {
    ...
    fetch(`/api/recipe/info/${id}`)
      ...
      .catch(() => {
        recipeDetailContent.innerHTML = '<p class="error-message">Error loading recipe.</p>';
      });
  }
...
refreshFavoritesBtn.addEventListener('click', () => {
    ...
    recipes.forEach(r => {
      ...
      card.innerHTML = `
        ...
        <div class="recipe-actions favorite-actions">
          <button class="action-btn view-fav-recipe" data-id="${r.recipe_id}">
            <i class="fas fa-eye"></i> View
          </button>
          <button class="action-btn edit-recipe" data-id="${r.recipe_id}" data-instr="${encodeURIComponent(r.instructions)}">
            <i class="fas fa-edit"></i> Edit
          </button>
          <button class="action-btn delete-recipe" data-id="${r.recipe_id}">
      ...
      imgEl.addEventListener('click', () => {
        if (isNumericId(r.recipe_id)) fetchRecipeDetails(r.recipe_id);
      ...
      card.querySelector('.delete-recipe')
        .addEventListener('click', e => {
          if (confirm('Delete this recipe?')) deleteFavorite(e.currentTarget.dataset.id);
        });
    });
  }
  ...
  addToFavoritesBtn.addEventListener('click', () => {
    ...
    fd.append('recipe_ingredients',
      (currentRecipe.extendedIngredients ?? []).map(i=>i.original).join('\n')
    );
    let instr = '';
    ...
    fetch(ADD_FAVORITE_URL, { method:'POST', body:fd })
      ...
      .then(res => {
        if (res.message) {
          ...
          showNotification(res.error || 'Failed.', true);
        }
      })
  ...
  function startTimer() {
    ...
    totalSeconds = m*60 + s;
    if (totalSeconds <= 0) {
      showNotification('Set a valid duration.', true);
      return;
    }
    isTimerRunning = true;
  ...
  const closeModalHandler = m => {
    ...
    if (m === priceBreakdownModal) {
      priceBreakdownImg.src = '';
      priceBreakdownData.innerHTML = '';
    }
  };
